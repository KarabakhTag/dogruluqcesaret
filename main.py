...
def button():
	BUTTON=[[InlineKeyboardButton(text="??????? Sahibim ",url="t.me/Thagiyevvvv")]]
	BUTTON+=[[InlineKeyboardButton(text="?? Open Source ??",url="https://github.com/karabakhtagg/karabakhdcbot")]]
...
async def _(client, message):
	...
	await message.reply_text(text="**Merhaba {}!**\n\n__Ben Pyrogram Api İle Yazılmış Eğlence Botuyum :)__\n\n**Repom =>** [Open Source](https://github.com/karabakhmusic/karabakhdcBot)\nDoğruluq mu? Cesaret mi? Oyun Komutu => /dc".format(
...
async def _(client, callback_query):
	...
	if str(user.id) == str(user_id):
		...
		if c_q_d == "d_data":
			await callback_query.answer(text="Doğruluq Sualı İstediniz", show_alert=False) # İlk Ekranda Uyarı Olarak Gösterelim
			await client.delete_messages(
				chat_id=callback_query.message.chat.id,
				message_ids=callback_query.message.message_id) # Eski Mesajı Silelim

			await callback_query.message.reply_text("**{user} Doğruluk Sorusu İstedi:** __{d_soru}__".format(user=user.mention, d_soru=d_sual)) # Sonra Kullanıcıyı Etiketleyerek Sorusunu Gönderelim
			return

		if c_q_d == "c_data":
			await callback_query.answer(text="Cesaret Sualı İstediniz", show_alert=False)
			await client.delete_messages(
				chat_id=callback_query.message.chat.id,
				message_ids=callback_query.message.message_id)
			await callback_query.message.reply_text("**{user} Cesaret Sualı İstedi:** __{c_soru}__".format(user=user.mention, c_soru=c_sual))
			return
...
async def _(client, message):
  ...
  if user.id in OWNER_ID:
    ...
    if MOD=="dekle":
      C_LİST.append(str(message.text))
      MOD=None
      await message.reply_text("**[?]** __Metin Dogruluq Sualı Olarak Eklendi!__")
      return
############################
